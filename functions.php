...
function getInvoiceId() {
    ...
    if ($result = $mysqli->query($query)) {
		...
		if ($row_cnt == 0) {
			...
			$invoice_number = 'B'.str_pad(INVOICE_INITIAL_VALUE, 4, '0', STR_PAD_LEFT);
		} else {
			// Increment the last invoice number
			$last_invoice_number = $row['invoice'];
			$last_number = (int)substr($last_invoice_number, 1);
			$new_number = $last_number + 1;
			$invoice_number = 'B'.str_pad($new_number, 4, '0', STR_PAD_LEFT);
		}
